...
⚠️ **Лицензия:** Код проекта открыт исключительно для демонстрации архитектурных решений и портфолио. Коммерческое использование и деплой на сторонние домены запрещены. Подробности в файле [LICENSE](LICENSE).
...
* 🔔 **Омниканальные уведомления:** Фоновые планировщики (Spring `@Scheduled`) отправляют красивые HTML-письма через **SMTP** (с интеграцией **Resend** в продакшене и Mailtrap для локальных тестов) и мгновенные пуши через интегрированного **Telegram-бота** (на базе паттернов Strategy/Factory) для напоминаний о собеседованиях и изменениях статусов откликов.
...
* 🔐 **Enterprise-Grade Безопасность:** Авторизация **OAuth2** (GitHub/Google) с бесшовным слиянием аккаунтов, безопасные сессии через **JWT** с HttpOnly Refresh токенами, Rate Limiting для AI-запросов (алгоритм Token Bucket на базе **Bucket4j**), AOP-аудит (Audit Trail) критических действий и **интеграция с Cloudflare** (защита от DDoS, WAF, SSL offloading, надежное восстановление реального IP пользователя через `CF-Connecting-IP` и **Email Routing** для бесплатного перенаправления входящей корпоративной почты).
...
* 📱 **Progressive Web App (PWA):** Устанавливайте приложение на свое устройство для нативного опыта, с оффлайн-кэшированием и отдельным окном.
