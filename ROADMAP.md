...
- [x] **Умный автопоиск вакансий hh.ru (Scraping/Public API):** поллинг вакансий по расписанию, проверка дубликатов, ИИ-сравнение с резюме (>80%) соавтоматической генерацией сопроводительного письма и Telegram-алертом.
- [x] **Расширенный автопоиск hh.ru:** добавление детальных фильтров (опыт, график, занятость, регион, только с зарплатой), индивидуальные интервалы опроса (15м-24ч), лояльные правила ИИ-матчера и полностью переработанное модальное окно.
- [x] **Исправление Proxy 407 (Cloud Run):** перенос системных свойств в статический инициализатор `CareerpilotAiApplication` и переход на изолированный `JdkClientHttpRequestFactory` с авторизацией для туннелирования.
...
**Примечание к релизам:** Следующий полноценный релиз (без альфа-версий) состоится после завершения всех целей визуальной части. После этого, следующим релизом будут выпущены все серверные цели.
...
- [x] **Kanban 2.0** — inline-редактирование заметок на карточке, фильтры по компании/дате, счётчик карточек в колонке, collapsed-режим.
