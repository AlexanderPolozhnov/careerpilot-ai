...
export default function AuthPages({ mode }: AuthPagesProps) {
    ...
    const schema = useMemo(() => {
        ...
        if (mode === 'reset-password') return resetSchema
        return forgotSchema
    }, [mode])

    const form = useForm<AuthFormValues>({
        resolver: zodResolver(schema) as unknown as Resolver<AuthFormValues>,
        defaultValues: { name: '', email: '', password: '', confirmPassword: '', newPassword: '' },
        mode: 'onSubmit',
    })
                    ...
                    <div className="w-full max-w-[400px]">
                            ...
                            onSubmit={form.handleSubmit(async (values) => {
                                ...
                                try {
                                    ...
                                    if (mode === 'reset-password') {
                                        ...
                                        const v = values as z.infer<typeof resetSchema>
                                        await resetPassword(token, v.newPassword)
                        ...
                        >
                            ...
                            {mode === 'reset-password' && (
                                <>
                                    <div className="space-y-2">
                                        <label className="block text-[13px] font-medium text-[#8b8fa3]">{t('auth.newPassword')}</label>
                                        <div className="relative">
                                            <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#6b7590]">
                                                <svg className="w-[18px] h-[18px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                            ...
                            >
                                ...
                                {mode === 'reset-password' && (form.formState.isSubmitting ? t('common.loading') : t('auth.resetPasswordButton'))}
                            </button>
