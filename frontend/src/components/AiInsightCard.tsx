...
export function AiInsightCard({ result, compact, className }: AiInsightCardProps) {
  ...
  return (
    ...
    >
      {/* Glow accent */}
      <div className="absolute top-0 left-0 w-full h-0.5 bg-gradient-to-r from-accent/60 via-accent/30 to-transparent" />
      ...
      <div className="flex items-start gap-3">
        ...
        {compact && (
          <ChevronRight className="w-4 h-4 text-ink-dim group-hover:text-ink transition-colors flex-shrink-0 mt-1" />
        )}
      </div>
    </div>
  )
