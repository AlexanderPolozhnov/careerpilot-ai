...
* 🔔 **Omnichannel Notifications:** Background schedulers (Spring `@Scheduled`) send beautiful HTML emails via **SMTP** (integrated with **Resend** in production, Mailtrap for local testing) and instant push notifications via an integrated **Telegram Bot** (utilizing Strategy/Factory patterns) for interview reminders and application status changes.
