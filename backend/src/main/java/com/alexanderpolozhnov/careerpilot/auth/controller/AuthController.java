...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public AuthResponse refresh(@CookieValue(name = "refresh_token") String refreshToken, HttpServletResponse response) {
    ...
    public void logout(@CookieValue(name = "refresh_token", required = false) String refreshToken, HttpServletResponse response) {
        ...
        ResponseCookie cookie = ResponseCookie.from("refresh_token", "")
                .httpOnly(true)
                .secure(false) // Assuming DEV might not use HTTPS, should be configurable but sticking to Spring defaults here or strictly false for dev. Wait, secure should probably just be omitted or dynamically set. Actually, secure is better if possible. I'll omit it so it works on localhost HTTP.
                .path("/api/auth")
                .maxAge(0)
                .sameSite("Strict")
                .build();
    ...
    private void setRefreshTokenCookie(HttpServletResponse response, String refreshToken) {
        ResponseCookie cookie = ResponseCookie.from("refresh_token", refreshToken)
                .httpOnly(true)
                .secure(false) // Omit secure for localhost support, ideally would be configured. Let's set secure=true later if needed.
                .path("/api/auth")
