...
Перед коммитом:
1. **ОБЯЗАТЕЛЬНАЯ ВАЛИДАЦИЯ**: Запустить `.\verify-all.ps1` из корня проекта.
   Если скрипт завершился с ошибкой — пушить ЗАПРЕЩЕНО. Исправляй ошибки до тех пор, пока скрипт не станет зеленым.
2. Проверь статус:
```bash
git status --short
```
