...
76. **OAuth2 Callback Error Handling**: При реализации OAuth2 callback обработки на фронтенде ВСЕГДА добавляйте валидацию токена перед сохранением (минимальная длина, не пустой), retry-логику для сетевых ошибок (3 попытки с задержкой) и понятные error сообщения с кнопками "Retry" и "Back to login". Заменяйте жесткий редирект `window.location.replace` на плавный `navigate` для лучшего UX. Централизуйте обработку OAuth токена через метод `handleOAuthCallback` в `AuthContext.tsx` с предварительной валидацией через `/auth/me`.
