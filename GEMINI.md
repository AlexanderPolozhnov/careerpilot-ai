...
4. **Файл** — путь к созданному TASK_PLAN.md`n81. **Null-checks for Enum fields in DTOs**: ╨Я╤А╨╕ ╨╖╨░╨╝╨╡╨╜╨╡ ╨╜╨░╤В╨╕╨▓╨╜╤Л╤Е `<select>` ╨╜╨░ ╨║╨░╤Б╤В╨╛╨╝╨╜╤Л╨╡ ╨║╨╛╨╝╨┐╨╛╨╜╨╡╨╜╤В╤Л, ╤Д╤А╨╛╨╜╤В╨╡╨╜╨┤ ╨╝╨╛╨╢╨╡╤В ╨╜╨░╤З╨░╤В╤М ╨┐╤А╨╕╤Б╤Л╨╗╨░╤В╤М `null` ╨┤╨╗╤П ╨╜╨╡╨▓╤Л╨▒╤А╨░╨╜╨╜╤Л╤Е ╨╖╨╜╨░╤З╨╡╨╜╨╕╨╣. ╨Т╤Б╨╡╨│╨┤╨░ ╨┤╨╛╨▒╨░╨▓╨╗╤П╪╣ ╤П╨▓╨╜╤Г╤О ╨┐╤А╨╛╨▓╨╡╤А╨║╤Г `if (request.field() != null)` ╨▓ ╤Б╨╡╤А╨▓╨╕╤Б╨╜╨╛╨╝ ╤Б╨╗╨╛╨╡ ╨▒╤Н╨║╨╡╨╜╨┤╨░, ╤З╤В╨╛╨▒╤Л ╨╕╨╖╨▒╨╡╨╢╨░╤В╤М ╨╛╤И╨╕╨▒╨╛╨║ 500 ╨┐╤А╨╕ ╨╝╨░╨┐╨┐╨╕╨╜╨│╨╡ ╨╕ ╤Б╨╛╤Е╤А╨░╨╜╨╡╨╜╨╕╨╕ ╨▓ ╨▒╨░╨╖╤Г ╨┤╨░╨╜╨╜╤Л╤Е.

83. **Синхронизация состояния фронтенда после внешних действий (Frontend State Sync after External Actions)**: При реализации функционала, требующего действий пользователя во внешней системе (например, OAuth-авторизация, переход по deep link в Telegram-бот и возврат обратно по кнопке "Готово"), простого вызова `queryClient.invalidateQueries()` часто недостаточно для немедленного обновления UI, если компонент полагается на локальное состояние формы (`react-hook-form`). В обработчиках таких событий необходимо явно запрашивать актуальные данные с бэкенда (через `queryClient.fetchQuery` или `await refetch()`) и принудительно обновлять значения формы (`form.setValue()`) и локальное состояние, чтобы интерфейс мгновенно отразил изменения без перезагрузки страницы.

84. **Явная регистрация Telegram-ботов в Spring Boot (TelegramBotsApi)**: При использовании базовой библиотеки `org.telegram:telegrambots` (а не spring-boot-starter, который часто отсутствует в Maven Central) недостаточно просто унаследовать `TelegramLongPollingBot` и пометить класс аннотацией `@Component` / `@Service`. Бин будет создан, но бот не начнет прослушивать обновления (Long Polling). Необходимо явно создать `@Bean` типа `TelegramBotsApi`, передать в него `DefaultBotSession.class` и вызвать `api.registerBot(yourBotHandler)`. Только после этого бот установит соединение с серверами Telegram.
85. **Надежное автоматическое переключение настроек (Fail-safe State Updates)**: Когда действие во внешней системе (например, привязка бота в Telegram) должно изменить настройку в приложении (например, провайдер уведомлений), не полагайтесь только на фронтенд-логику после возвращения пользователя. Делайте авто-переключение на самом бэкенде в момент подтверждения внешнего действия (в обработчике `/start`). Фронтенд должен служить лишь для синхронизации UI, запрашивая свежие данные без кэша (например, прямой API call вместо `fetchQuery`), чтобы избежать рассинхронизации, если пользователь закроет вкладку до нажатия кнопки подтверждения.
### ⚠️ Известные ошибки и паттерны

- **Telegram Bot в тестах:** При использовании @SpringBootTest необходимо убедиться, что telegram.bot.enabled=false в application-test.yaml. Иначе инициализация Telegram API завершится ошибкой из-за отсутствия валидного токена, что обрушит контекст приложения.
- **CI Pipeline:** Интеграционные тесты, требующие Docker (Testcontainers), могут быть нестабильны или медленны в GitHub Actions. Текущая политика — исключать их из основного пайплайна через -Dtest='!*Tests', оставляя только чистые Unit-тесты.
- **SecurityContext в тестах:** При тестировании сервисов, использующих SecurityContextHolder (через CurrentUserResolver), необходимо всегда вызывать SecurityContextHolder.clearContext() в @AfterEach, чтобы избежать влияния состояния на другие тесты в том же процессе.
- **Lombok @Data в Entities:** Избегай @Data в JPA сущностях из-за проблем с hashCode/equals и бесконечной рекурсией в связях. Используй @Getter/@Setter.
